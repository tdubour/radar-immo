...
window.addEventListener("message", async (event) => {
  ...
  state.radarStatus = {
    ...
    sources: mergeExtensionSourceStatuses(state.radarStatus.sources || [], payload),
    error: ""
  };
  persist();
...
async function refreshRemoteRadar() {
  try {
    ...
    state.radarStatus = { loading: false, connected: Boolean(health.ok) || extensionRadarListings.length > 0, extensionCount: extensionRadarListings.length, lastRun: health.generatedAt ? { started_at: health.generatedAt } : null, sources: mergeExtensionSourceStatuses(health.sources || [], extensionRadarPayload), error: health.ok || extensionRadarListings.length ? "" : health.message };
    persist();
    if (state.page === "radar") render();
  } catch (error) {
